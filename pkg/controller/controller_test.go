...
func TestDefaultConfigDoesNotSkipRecreate(t *testing.T) {
	...
	got, err := prepareController(clientset, ns, ns, tweakopts, &Flags{SkipRecreate: false}, ssc, keyRegistry)
	if err != nil {
...
func TestSkipRecreateConfigDoesSkipIt(t *testing.T) {
	...
	got, err := prepareController(clientset, ns, ns, tweakopts, &Flags{SkipRecreate: true}, ssc, keyRegistry)
	if err != nil {
		t.Fatalf("err %v want %v", got, nil)
	}
	if got == nil {
